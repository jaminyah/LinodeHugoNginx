...
There are now two piplines in the workflow. `build` and `deploy-production`.
...
For Wercker to access our Linode server, a Public Key and Private Key pair needs to be generated by the Werker app. Enter the name `linode` for the keys.

<p align="center">
  <img src="/images/wercker/ssh_key.jpg" alt="Create SSH" /> 
</p>

Successful generation of the key-pair should show the public key. The private key will not be visible.
